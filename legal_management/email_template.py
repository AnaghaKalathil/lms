...
def create_from_frontend(template_name, mjml_source):
    """
    Create Email Template from frontend MJML builder
    """
    ...
    html_content = compile_mjml_to_html(mjml_source)

    # Replace local /files/ with absolute URLs
    
    html_content = html_content.replace('src="/files/', f'src="{get_url("/files")}/')
